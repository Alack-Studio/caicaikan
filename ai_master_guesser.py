...
def ask_ai(inp=None, is_hidden=False):
    ...
    if st.session_state.role == "AI 猜":
        sys_prompt = "身份：侦探。目标：猜用户想的名人。第一句话直接问是非题。严禁废话。猜中回复：答案是：[人名]。"
    ...
    # --- 我猜模式 [修复重点] ---
        ...
        sys_prompt = (
            ...
            "指令列表（严格按优先级执行）：\n"
            "1. 【最高优先级】若用户说'认输'、'揭晓'、'猜不到'：必须立即停止游戏，直接回复：'很遗憾。真相是：[人名]。' (严禁鼓励或给新线索！)\n"
            "2. 若用户索要'提示'或'线索'：直接给出一个新的描述性线索。严禁输出'分支触发'等字样，严禁回答'是/否'。\n"
            "3. 若用户在提问特征（带问号）：只回答'是'、'否'或'模糊'。\n"
            "4. 若用户猜中名字：回复'🎉 恭喜你，答对了！真相是：[人名]'"
        )
    ...
    with st.spinner("..."):
        try:
            ...
            if any(k in reply for k in ["答案是", "恭喜", "真相是"]):
                ...
                st.session_state.win = not (inp and any(k in str(inp) for k in ["认输", "揭晓"]))
...
if not st.session_state.started:
    ...
    if not st.session_state.over:
        if st.session_state.role == "AI 猜":
            ...
            with c2:
                if st.button("🙅 猜不到"): st.session_state.pending = "我认输，请直接揭晓答案，不要再给线索了。"; st.rerun()
            with c3:
                if st.button("🔄 换个人"):
